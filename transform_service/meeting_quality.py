...
pure scoring logic here, all graph I/O delegated to ``memgraph_client``.
...
def score_attendance_ratio(attended: Optional[int], invited: Optional[int]) -> Optional[float]:
    if not invited or invited <= 0 or attended is None:
        return None
    return round(max(0.0, min(1.0, attended / invited)), 4)


def _per_hour(count: int, duration_minutes: Optional[float]) -> Optional[float]:
...
def score_yield(
    count: int, duration_minutes: Optional[float], population_rates: Sequence[float]
) -> Optional[float]:
...
def score_action_completion(done: Optional[int], total: Optional[int]) -> Optional[float]:
    if not total or total <= 0:
        return None
    return round(max(0.0, min(1.0, (done or 0) / total)), 4)


def score_agenda_present(text: Optional[str]) -> Optional[float]:
...
def score_recurrence_health(recent_scores: Sequence[Optional[float]]) -> Optional[float]:
...
def composite_quality(
    components: Dict[str, Optional[float]], weights: Optional[Dict[str, float]] = None
) -> Optional[float]:
...
async def score_all_meetings() -> Dict[str, Any]:
    """Score every meeting and persist results. Reports data-signal coverage honestly."""
    rows = await memgraph_client.get_meetings_quality_inputs()
    # Build population distributions of per-hour rates (only meetings with a duration).
    decision_rates = [
        r for r in (_per_hour(row["n_decisions"], row["duration_minutes"]) for row in rows) if r is not None
    ]
    action_rates = [
        r for r in (_per_hour(row["n_actions"], row["duration_minutes"]) for row in rows) if r is not None
    ]
    ...
    insufficient = 0
    for row in rows:
        result = compute_quality(row, population)
        ...
        if score is None:
            ...
            scored += 1
        await memgraph_client.set_meeting_quality(
            row["id"], score, {k: v for k, v in components.items()}  # type: ignore[union-attr]
        )
    summary = {"total": len(rows), "scored": scored, "insufficient_data": insufficient}
    log.info("meeting_quality.scored_all", **summary)
    return summary
