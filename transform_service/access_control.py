...
class Scope:
    ...
    value: Optional[str] = None  # team name / project key; None for all/org
...
class Principal:
    ...
    allowed_scopes: tuple = ()  # explicit scope tokens; empty => derived from role/team
...
def load_policy(path: Optional[str] = None) -> Dict[str, Principal]:
    """Load the principal policy map (YAML file if configured, else the in-code default)."""
    path = path or os.environ.get("ACCESS_POLICY_FILE")
    raw: Dict[str, Dict]
...
def resolve_principal(name: str, policy: Optional[Dict[str, Principal]] = None) -> Principal:
...
def authorize(name: str, requested_scope: str, policy: Optional[Dict[str, Principal]] = None) -> Scope:
...
def aggregates_only(name: str, requested_scope: str, policy: Optional[Dict[str, Principal]] = None) -> bool:
...
def scope_predicate(scope: Scope) -> Dict[str, Optional[str]]:
...
def visible_scopes(name: str, policy: Optional[Dict[str, Principal]] = None) -> List[str]:
